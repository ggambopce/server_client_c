...
void* handle_client(void* arg) {
    ...
    printf("클라이언트 연결 성공 %s", inet_ntoa(client_address.sin_addr));
    ...
    while ((string_length = read(client_socket, message, BUF_SIZE)) > 0)
    {
        ...
        if (temp_ptr && hum_ptr && id_ptr)
        {
            ...
            sscanf(id_ptr,   "ID=%d",  &device_id);

            // 추출한 값을 JSON 형식으로 출력
            printf("파싱된 JSON: { \"temperature\": %.2f, \"humidity\": %.2f, \"device_id\": %d }\n", temp, hum, device_id);
...
int main(int argc, char *argv[])
{
    ...
    while(1) {
        ...
        pthread_t t_id;
        pthread_create(&t_id, NULL, handle_client, args);
        pthread_detach(t_id);
    }    
